...
public class BMPImageWriter extends ImageWriter implements Constants{
  ...
  int getHeaderSize(){
    ...
    size+=getColorsInPalette();              // either colour table
    size+=(compression==BI_BITFIELDS)?12:0;  // or colour masks or none of them
    return size;
  }

  private void writeHeader(ImageOutputStream out,BufferedImage image)throws IOException{
  ...
  public void write(ImageOutputStream out,BufferedImage image)throws IOException{
  ...
  public void write01bit(ImageOutputStream out,BufferedImage image,IndexColorModel icm)throws IOException{
  ...
  public void write04bit(ImageOutputStream out,BufferedImage image,IndexColorModel icm)throws IOException{
  ...
  public void write08bit(ImageOutputStream out,BufferedImage image,IndexColorModel icm)throws IOException{
  ...
  private void write16bit(ImageOutputStream out,BufferedImage image)throws IOException{
  ...
  public void write24bit(ImageOutputStream out,BufferedImage image)throws IOException{
  ...
  private void write32bit(ImageOutputStream out,BufferedImage image)throws IOException{
  ...
  public void write(IIOMetadata streamMetadata,IIOImage img,ImageWriteParam param)throws IOException{
    ...
    if(!(img.getRenderedImage() instanceof BufferedImage)){
      throw new IOException("mmsc BMPImageWriter : Can only write BufferedImage objects");
    }
    BufferedImage image=(BufferedImage)img.getRenderedImage();
    ColorModel cm=image.getColorModel();
    if(cm instanceof IndexColorModel){
      IndexColorModel icm=(IndexColorModel)cm;
      int colCount=icm.getMapSize();
      if(colCount==2){         write01bit(out,image,icm);
      }else if(colCount<=16){  write04bit(out,image,icm);
      }else if(colCount<=256){ write08bit(out,image,icm);
      }else{                   write(out,image);
      }
    }else{
      switch(image.getType()){
      case BufferedImage.TYPE_USHORT_555_RGB:  write16bit(out,image);break;
      case BufferedImage.TYPE_3BYTE_BGR:       write24bit(out,image);break;
      case BufferedImage.TYPE_INT_RGB:         write32bit(out,image);break;
      default:                                 write(out,image);break;
      }
    }
  }
