...
public class BMPImageReader extends ImageReader implements Constants{
...
//    l.add(ImageTypeSpecifier.createFromBufferedImageType(BufferedImage.TYPE_INT_RGB));
  ...
  private void checkIndex(int imageIndex) {
    if (imageIndex != 0) {
      throw new IndexOutOfBoundsException("bad index in bmp image reader");
    }
  }
  ...
  private void readHeader(ImageInputStream in)throws IOException{
    ...
    if (headerSize == 12) {
      ...
      bitCount = in.readUnsignedShort();  // 1,4,8,16,24,32
...
//        colortable[i]=in.readInt();
      ...
      icm=new IndexColorModel(bitCount,coloursUsed,redCols,greenCols,blueCols);
    }
...
/*  
  private BufferedImage read(ImageInputStream in)throws IOException{
    readHeader(in);
    int scanlineSize = ((width*bitCount+31)/32)*4;
